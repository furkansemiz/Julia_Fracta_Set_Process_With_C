...
void readPGMFormattedImage( PGMFormattedImage *pgmFormattedImagePointer ){
    FILE *pgmFILE = NULL;
    if( ( pgmFILE = fopen( "./bos.pgm", "r" ) ) != NULL ){
...
void writePGMFormattedImage( PGMFormattedImage* pgmFormattedImagePointer ){
    FILE *pgmFILE = NULL;
    if( ( pgmFILE = fopen( "./Processed_Batman_2048.pgm", "w" ) ) != NULL ){
        ...
        fprintf( pgmFILE, "%d %d\n", *( pgmFormattedImagePointer->widthOfImage ), *( pgmFormattedImagePointer->heightOfImage ) );
        //writing width and height of pgm formatted file
        ...
        for( int rowIndex = 0; rowIndex < *( pgmFormattedImagePointer->heightOfImage ); rowIndex ++ ){
            ...
            if( rowIndex != *( pgmFormattedImagePointer->heightOfImage ) - 1 ){
                fprintf( pgmFILE, "\n" );
            }
        }
